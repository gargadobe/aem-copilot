        ...
        await vscode.window.withProgress({
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                try {
                    ...
                    const document = await vscode.workspace.openTextDocument(newFilePath); // open the document
                    const editor = await vscode.window.showTextDocument(document); // show the document in the editor
                    await vscode.commands.executeCommand('editor.action.formatDocument'); // format the document
                }
                progress.report({ increment: 100.0 / files.length, message: `Creating file: ${file.path}` });
            }
        });
    }
}
...
export function parseFileStringWithFilenames(str: string): File[] {
    ...
    if (matches) {
        for (let match of matches) {
            ...
            if(content && (content.startsWith('javascript') || content.startsWith('css'))) {
